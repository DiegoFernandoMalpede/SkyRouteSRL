...
(1, 1, 1, '2025-06-06', 20000, 'Activa'),
(2, 3, 2, '2025-06-06', 30000, 'Activa'),
(3, 2, 3, '2025-06-07', 35000, 'Activa');

#Botón de arrepentimiento
INSERT INTO boton_arrepentimiento (id_ventas, fecha_anulacion, motivo_anulacion) VALUES
(2, '2025-06-06', 'Error en el destino');

#consultas SQL (lenguaje DML - SELECT)

#Listar todos los clientes 
SELECT * FROM clientes;

#Mostrar ventas realizadas el 2025-06-06
SELECT * FROM ventas 
WHERE DATE(fecha_ventas) = '2025-06-06';

#Obtener la última venta de cada cliente y su fecha
SELECT ventas.id_cliente, MAX(ventas.fecha_ventas) AS ultima_venta
FROM ventas 
GROUP BY ventas.id_cliente;

#Buscar todos los destinos que empiezan con “R”
SELECT * FROM destinos 
WHERE ciudad LIKE 'R%';

#Ventas realizadas por pais
SELECT destinos.pais, COUNT(ventas.id_ventas) AS cantidad_ventas
FROM ventas 
JOIN destinos ON ventas.id_destino = destinos.id_destino
GROUP BY destinos.pais;